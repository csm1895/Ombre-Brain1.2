    ...
    for line in f.read_text(encoding="utf-8").splitlines():
        ...
        if (not target or target == reader) and reader not in read_by:
            ...
            if mark_read:
                ...
                changed = True
        items.append(("__ALL__", item))

    real_items = [x for x in items if not isinstance(x, tuple)]

    if changed:
        all_items = [x[1] for x in items if isinstance(x, tuple)]
        f.write_text(
